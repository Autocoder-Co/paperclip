...
export function renderPaperclipWakePrompt(
  ...
  const lines = resumedSession
      ? [
        ...
        "",
        "Paperclip issue-state contract: this wake prompt governs Paperclip issue state, liveness, and routing mechanics only; follow the agent, company, repo, and issue instructions for methodology, planning, implementation, and verification. Handle actionable wake work in this heartbeat when those instructions and gates allow it. Leave durable progress and then give the issue a clear final disposition before ending the heartbeat: `done`, `in_review` with a real reviewer/approval/interaction path, `blocked` with first-class blockers or a named unblock owner/action, delegated follow-up issues with blockers, or `in_progress` only when a live continuation path exists. Use child issues for long or parallel delegated work instead of polling. Comments, documents, screenshots, work products, and `Remaining` bullets are evidence, not valid liveness paths by themselves.",
        "",
